...
describe('useGifs', () => {
    ...
    test('should return default values and methods', () => {
        ...
        expect(result.current.handleTermClickked).toBeDefined();
    ...
    test('should return a list of gifs where handleTermClicked is called', async () => {
        // handleSearch
        const { result } = renderHook(() => useGifs());

        await act(async () => {
            await result.current.handleTermClickked('goku');
        });

        expect(result.current.gifs.length).toBe(10);
    });

    test('should return a list of gifs from de cache', async () => {
        const { result } = renderHook(() => useGifs());
    ...
    test('should return no more than 8 previos terms', async () => {
